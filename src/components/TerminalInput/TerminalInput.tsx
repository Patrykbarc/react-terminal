...
export function TerminalInput({
  ...
  const buttonCursorStyle =
    inputTrimLength > 0 ? 'cursor-pointer' : 'cursor-default'
  const sendMessageIconStyle =
    inputTrimLength > 0 ? 'text-sky-600' : 'text-gray-400'
  ...
  return (
    <div className="flex items-center border-t pl-3">
      <input
        ...
        onKeyUp={(event) =>
          handleSetChat({ event, useStates: { input, setInput, setChat } })
        }
      />
      <button
